...
class Benchmark:
    ...
    def benchmark_cv_model(self, model_name="resnet50", device="cpu", num_runs=10):
        ...
        # Warmup: try calling model with (image, gabarits), fall back to (image) if signature differs
        model_accepts_gabarits = False
        with torch.no_grad():
            try:
                _ = model(dummy_image, dummy_gabarits)
                model_accepts_gabarits = True
            except TypeError:
                _ = model(dummy_image)
